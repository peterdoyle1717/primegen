...
with three hand-seeded base cases

    prime(4) = {tetrahedron},   prime(5) = ∅,   prime(6) = {octahedron}.

`buckygen` produces no fullerene-dual seeds at v ≤ 6 that satisfy
primegen's max-degree-6 filter; the three small primes that exist must
be inserted by hand.  See [`docs/PRIMEGEN_BASE_CASES.md`](docs/PRIMEGEN_BASE_CASES.md).
